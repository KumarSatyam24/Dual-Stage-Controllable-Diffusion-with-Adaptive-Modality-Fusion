#!/usr/bin/env python3
"""
LPIPS Metric Test on Full Sketchy Test Set

Computes Learned Perceptual Image Patch Similarity using AlexNet features.
LPIPS measures perceptual similarity, lower values indicate more similar images.
Typical range: 0.0 to 1.0, where 0.0 means identical.

Usage:
    python test_metrics/test_lpips_full.py \
        --stage1_checkpoint /workspace/checkpoints/stage1/epoch_18.pt \
        --stage2_checkpoint /workspace/checkpoints/stage2/epoch_6.pt \
        --output_dir ./results/lpips_test \
        --num_samples -1
"""
...
class LPIPSValidator:
    """Standalone LPIPS metric validator for full test set."""
    ...
    def __init__(
        ...
        if not LPIPS_AVAILABLE:
            raise RuntimeError("LPIPS is required but not installed. Run: pip install lpips")
    ...
    def setup_lpips(self):
        ...
        print(f"   ✅ LPIPS ({self.lpips_net}) ready")
        print()
    ...
    def compute_lpips(self, generated, ground_truth):
        ...
        gen_lpips = (generated * 2 - 1).to(self.device)
        gt_lpips = (ground_truth * 2 - 1).to(self.device)
        lpips_score = self.lpips_model(gen_lpips, gt_lpips).mean().item()
        return lpips_score

    def validate(self):
        """Run validation on full test set."""
        print("🔄 Starting LPIPS validation...")
        ...
        for batch_idx in tqdm(range(num_batches), desc="Computing LPIPS"):
            ...
            try:
                ...
                lpips_stage2 = self.compute_lpips(generated, photos_gt)
                lpips_stage1 = self.compute_lpips(stage1, photos_gt)

                self.results['stage2_lpips_scores'].append(lpips_stage2)
                self.results['stage1_lpips_scores'].append(lpips_stage1)
                ...
                for i, file_id in enumerate(file_ids):
                    self.results['per_sample_results'].append({
                        ...
                        'prompt': prompts[i],
                        'stage2_lpips': lpips_stage2,
                        'stage1_lpips': lpips_stage1
                    })
        ...
        print(f"\n✅ LPIPS validation complete in {elapsed/3600:.2f} hours")
    ...
    def save_intermediate_results(self):
        ...
        with open(temp_path, 'w') as f:
            json.dump({
                'stage2_lpips_mean': float(np.mean(self.results['stage2_lpips_scores'])) if self.results['stage2_lpips_scores'] else 0,
                'stage1_lpips_mean': float(np.mean(self.results['stage1_lpips_scores'])) if self.results['stage1_lpips_scores'] else 0,
                'num_samples': len(self.results['per_sample_results'])
            }, f, indent=2)

    def save_results(self, elapsed_time):
        """Save final results."""
        print("\n💾 Saving LPIPS results...")

        summary = {
            'stage2_lpips': {
                'mean': float(np.mean(self.results['stage2_lpips_scores'])),
                'std': float(np.std(self.results['stage2_lpips_scores'])),
                'min': float(np.min(self.results['stage2_lpips_scores'])),
                'max': float(np.max(self.results['stage2_lpips_scores']))
            },
            'stage1_lpips': {
                'mean': float(np.mean(self.results['stage1_lpips_scores'])),
                'std': float(np.std(self.results['stage1_lpips_scores'])),
                'min': float(np.min(self.results['stage1_lpips_scores'])),
                'max': float(np.max(self.results['stage1_lpips_scores']))
            },
            'lpips_improvement': float(np.mean(self.results['stage1_lpips_scores']) - np.mean(self.results['stage2_lpips_scores']))
        }

        results_path = self.output_dir / "lpips_results.json"
        with open(results_path, 'w') as f:
            json.dump({
                ...
                'config': {
                    ...
                    'lpips_network': self.lpips_net
                },
                'execution_time_seconds': elapsed_time,
                'timestamp': datetime.now().isoformat()
            }, f, indent=2)

        csv_path = self.output_dir / "per_sample_lpips.csv"
        ...
        print("\n" + "=" * 70)
        print("📊 LPIPS VALIDATION RESULTS")
        print("=" * 70)
        print(f"\nStage 1 LPIPS: {summary['stage1_lpips']['mean']:.4f} ± {summary['stage1_lpips']['std']:.4f}")
        print(f"Stage 2 LPIPS: {summary['stage2_lpips']['mean']:.4f} ± {summary['stage2_lpips']['std']:.4f}")
        print(f"Improvement:   {summary['lpips_improvement']:.4f} (lower is better)")
        print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="LPIPS Metric Test on Full Sketchy Test Set")
    parser.add_argument("--stage1_checkpoint", type=str, required=True)
    parser.add_argument("--stage2_checkpoint", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="./results/lpips_test")
