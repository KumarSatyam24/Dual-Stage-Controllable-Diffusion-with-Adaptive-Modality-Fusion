#!/usr/bin/env python3
"""
PSNR Metric Test on Full Sketchy Test Set

Computes Peak Signal-to-Noise Ratio between generated and ground truth images.
PSNR is measured in dB, higher values indicate better quality (typical range: 20-40 dB).

Usage:
    python test_metrics/test_psnr_full.py \
...
class PSNRValidator:
    """Standalone PSNR metric validator for full test set."""
    ...
    def __init__(
        ...
        print("=" * 70)
        print("📊 PSNR Metric Validation - Full Sketchy Test Set")
        print("=" * 70)
    ...
    def validate(self):
        ...
        print("🔄 Starting PSNR validation...")
        ...
        for batch_idx in tqdm(range(num_batches), desc="Computing PSNR"):
            ...
            try:
                ...
                psnr_stage2 = self.compute_psnr(generated, photos_gt)
                psnr_stage1 = self.compute_psnr(stage1, photos_gt)

                self.results['stage2_psnr_scores'].append(psnr_stage2)
                self.results['stage1_psnr_scores'].append(psnr_stage1)
                ...
                for i, file_id in enumerate(file_ids):
                    self.results['per_sample_results'].append({
                        ...
                        'prompt': prompts[i],
                        'stage2_psnr': psnr_stage2,
                        'stage1_psnr': psnr_stage1
                    })
        ...
        print(f"\n✅ PSNR validation complete in {elapsed/3600:.2f} hours")
    ...
    def save_intermediate_results(self):
        ...
        with open(temp_path, 'w') as f:
            json.dump({
                'stage2_psnr_mean': float(np.mean(self.results['stage2_psnr_scores'])) if self.results['stage2_psnr_scores'] else 0,
                'stage1_psnr_mean': float(np.mean(self.results['stage1_psnr_scores'])) if self.results['stage1_psnr_scores'] else 0,
    ...
    def save_results(self, elapsed_time):
        ...
        summary = {
            'stage2_psnr': {
                'mean': float(np.mean(self.results['stage2_psnr_scores'])),
                'std': float(np.std(self.results['stage2_psnr_scores'])),
                'min': float(np.min(self.results['stage2_psnr_scores'])),
                'max': float(np.max(self.results['stage2_psnr_scores']))
            },
            'stage1_psnr': {
                'mean': float(np.mean(self.results['stage1_psnr_scores'])),
                'std': float(np.std(self.results['stage1_psnr_scores'])),
                'min': float(np.min(self.results['stage1_psnr_scores'])),
                'max': float(np.max(self.results['stage1_psnr_scores']))
            },
            'psnr_improvement': float(np.mean(self.results['stage2_psnr_scores']) - np.mean(self.results['stage1_psnr_scores']))
        }

        results_path = self.output_dir / "psnr_results.json"
        ...
        csv_path = self.output_dir / "per_sample_psnr.csv"
        ...
        print("\n" + "=" * 70)
        print("📊 PSNR VALIDATION RESULTS")
        print("=" * 70)
        print(f"\nStage 1 PSNR: {summary['stage1_psnr']['mean']:.2f} ± {summary['stage1_psnr']['std']:.2f} dB")
        print(f"Stage 2 PSNR: {summary['stage2_psnr']['mean']:.2f} ± {summary['stage2_psnr']['std']:.2f} dB")
        print(f"Improvement:  {summary['psnr_improvement']:.2f} dB")
        print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="PSNR Metric Test on Full Sketchy Test Set")
    parser.add_argument("--stage1_checkpoint", type=str, required=True)
    parser.add_argument("--stage2_checkpoint", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="./results/psnr_test")
