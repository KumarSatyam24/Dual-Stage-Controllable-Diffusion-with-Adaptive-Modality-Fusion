#!/usr/bin/env python3
"""
Regenerate all 125 categories with optimal settings:
- Guidance scale: 2.5 (better sketch fidelity)
- Minimal prompts: Just category name (no extra details)
- Focus: Preserve sketch structure, avoid adding extra objects
"""
...
def load_model(checkpoint_path, device, guidance_scale=2.5):
    """Load the model with specified guidance scale."""
    print(f"📦 Loading Stage 1 model with guidance_scale={guidance_scale}...")
    ...
    # Create pipeline with optimal guidance scale
    pipeline = Stage1DiffusionPipeline(
        model,
        num_inference_steps=30,
        guidance_scale=guidance_scale,  # Using optimal value
        device=device
    )
    
    return model, pipeline

def generate_minimal_prompt(category):
    """
    Generate minimal prompt - just the category name.
    Stage 1 should focus on sketch structure, not complex details.
    """
    # Remove underscores and numbers
    clean_name = category.replace('_', ' ')
    
    # Just use "a <category>" - no extra details
...
def create_comparison_grid(results, grid_num, output_dir):
    ...
    # Save grid
    grid_path = os.path.join(output_dir, f"optimized_grid_{grid_num:02d}.png")
    grid.save(grid_path)
...
def main():
    print("=" * 70)
    print("🎨 REGENERATING ALL 125 CATEGORIES WITH OPTIMAL SETTINGS")
    print("=" * 70)
    print("\n⚙️  Configuration:")
    print("   - Guidance Scale: 2.5 (better sketch fidelity)")
    print("   - Prompts: Minimal (category name only)")
    print("   - Focus: Preserve sketch structure exactly")
    print("   - Goal: Single object matching sketch, no extras")
    print()
    ...
    # Load model with optimal guidance scale (2.5)
    model, pipeline = load_model(checkpoint_path, device, guidance_scale=2.5)
    
    # Create output directory
    output_dir = "test_outputs_optimized"
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"\n🎨 Generating images with optimal settings...")
    print("=" * 70)
    ...
    for idx, category in enumerate(categories, 1):
        try:
            ...
            # Generate MINIMAL prompt (just category name)
            prompt = generate_minimal_prompt(category)
            
            print(f"🎨 [{idx}/{len(categories)}] {category} ('{prompt}')...", end=" ", flush=True)
    ...
    print("\n" + "=" * 70)
    print("📊 Creating optimized comparison grids...")
    print("=" * 70)
    ...
    print(f"\n⚙️  Settings used:")
    print(f"   Guidance Scale: 2.5 (optimal for sketch fidelity)")
    print(f"   Prompts: Minimal (category name only)")
    print(f"\n📂 Output directory: {output_dir}/")
    print(f"   - Individual images: {output_dir}/<category>.png")
    print(f"   - Comparison grids: {output_dir}/optimized_grid_XX.png")
    
    # Copy grids to workspace
    print("\n📋 Copying optimized grids to workspace...")
    for i in range(1, min(4, grids_created + 1)):
        src = f"{output_dir}/optimized_grid_{i:02d}.png"
        dst = f"/workspace/optimized_grid_{i:02d}.png"
    ...
    print("🔍 COMPARISON:")
    print("=" * 70)
    print("Old grids (guidance=7.5, detailed prompts):")
    print("   - /workspace/comparison_grid_XX.png")
    print("   - Issues: Extra objects, changed structure, too detailed")
    print()
    print("New grids (guidance=2.5, minimal prompts):")
    print("   - /workspace/optimized_grid_XX.png")
    print("   - Expected: Better sketch fidelity, single objects, cleaner")
    print()
    print("👉 Compare the old vs new grids to see the improvement!")
